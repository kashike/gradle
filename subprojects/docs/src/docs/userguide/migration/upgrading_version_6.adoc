...
==== Using 'master' subdirectory to define root build

Using a special directory named `master` to define the project root of a multi-project build has been deprecated.
This feature allowed Gradle to recognize the multi-project build when invoked from a subdirectory of such build.
